...
## O que foi desenvoldido

O projeto shopping cart é o primeiro projeto onde realizei o consumo de uma API para a construção de um site, com um CSS já pré-definido, renderizando as informações da API em cards individuais.

A API consumida aqui é a do [Mercado Livre](https://api.mercadolibre.com/sites/MLB/search?q=computador)

É necessário limpar o localStorage antes de testar a aplicação.

Aperte F12 no seu navegador com a página do projeto carregada e escreva o seguinte comando no console:
...
A pessoa deve digitar o nome dela no input 'Nome'.
...
A pessoa deve marcar ou desmarcar o campo.
